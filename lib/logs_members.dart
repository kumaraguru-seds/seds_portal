...
class _LogsMembersPageState extends State<LogsMembersPage>
  ...
  Future<void> _startWork() async {
    ...
    if (u == null) return;
    try {
      // ── Fix: GPS service must be ON before we even ask for permission ──
      final bool serviceEnabled = await Geolocator.isLocationServiceEnabled();
      if (!serviceEnabled) {
        _showSnack(
                        ...
                        children: [
                          if (_isWorking) ...[
                            ...
                            if (_requestStatus == 'requested') ...[
                              ...
                              const SizedBox(height: 4),
                              Text('Your start session request was declined.', style: poppins(fontSize: 13, color: Colors.redAccent, fontWeight: FontWeight.bold)),
                              const SizedBox(height: 20),
                              SizedBox(
                                width: double.infinity,
                                child: ElevatedButton.icon(
